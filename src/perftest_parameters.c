...
static void usage(const char *argv0, VerbType verb, TestType tst, int connection_type)
{
	...
	if (tst != FS_RATE) {
		...
		if (cuda_memory_supported()) {
			...
			printf(" GPU->host bounce buffer: alloc GPU buf + pinned host buf, cudaMemcpy before each RDMA post.\n");
			printf("      Use to benchmark full GPU->CPU->RDMA pipeline without GPUDirect.\n");
			printf("      Server must have the GPU; client uses plain host memory.\n");
			...
			case 0: /* required for long options to work. */
				...
				if (user_param->memory_type != MEMORY_HOST &&
				    (mmap_file_flag || use_mlu_flag || use_neuron_flag || use_hl_flag ||
				     ...
				     (use_cuda_bounce_flag && user_param->memory_type != MEMORY_HOST && user_param->memory_type != MEMORY_CUDA_BOUNCE))) {
